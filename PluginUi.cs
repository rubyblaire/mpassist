...
public sealed class PluginUi
{
    ...
    public void Draw()
    {
        ...
        ImGui.SetNextWindowSize(new Vector2(920, 720), ImGuiCond.FirstUseEver);
    ...
    private void DrawHeader()
    {
        ...
        ImGui.TextWrapped("A marketboard shopping companion for MakePlace lists. It tells you what to search next, ranks visible listings, warns on gil caps, tracks purchases, and advances the queue after your manual buy.");
        ImGui.Spacing();
    ...
    private void DrawCurrentItemPanel()
    {
        ...
        ImGui.Text($"Needed: {current.QuantityNeeded}    Owned: {current.QuantityOwned}    Remaining: {current.QuantityRemaining}");
        ...
        {
            ImGui.Spacing();
            ImGui.TextWrapped("Search this item on the in-game marketboard. When listings arrive, the assistant will rank them here.");
            return;
        }
    ...
    private void DrawShoppingList()
    {
        ...
        if (!ImGui.BeginTable("##shoppingItems", 7, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.ScrollY, new Vector2(-1f, 250f)))
            return;
        ...
        foreach (var item in this.shoppingService.ShoppingList)
        {
            ...
            ImGui.TableSetColumnIndex(5);
            ImGui.Text(item.HasSeenListings ? "Seen" : "—");

            ImGui.TableSetColumnIndex(6);
            var buttonLabel = $"Focus##{item.DisplayName}_{item.ItemId}";
